...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Insert/upsert into listings table
    const { error: insertError } = await supabase.from("listings").upsert({
    ...
    if (insertError) {
      console.error('Error upserting listing:', insertError);
      throw new Error(`Failed to upsert listing: ${insertError.message}`);
    }
...
function extractHouseRules(raw: any, section: string) {
  const groups = raw.house_rules?.general ?? [];
  const match = groups.find((g: any) => g.title === section);
